...
function compress(chars){
    ...
    output = Array.from(map.entries(), ([key, value]) => key + value).join("");
    for(let j = 0; j < output.length; j++){
        if(Number(output[j]) == 1 && isNaN(output[j + 1])){
            result += "";
        } else {
            result += output[j];
        }
...
for(let j = 0; j < evens.length; j++){
            map.set(evens[j], (map.get(evens[j]) ?? 0) + 1);
            if(map.get(evens[j]) > maxCount){
                maxCount = map.get(evens[j]);
            }
        }
    }
